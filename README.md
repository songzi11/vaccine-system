...
本项目是一款面向社区医疗服务场景的数字化疫苗接种管理平台，采用前后端分离架构，支持多端协同操作。系统涵盖疫苗库存精细化管理、智能预约调度、接种全流程追溯、统计决策分析等核心能力，满足管理员、医生、居民三类角色的业务需求。
...
| 库存管理 | 总仓批次入库、接种点库存调拨、FEFO 批次分配、库存预警 |
...
| `appointment` | 预约单表 | id, user_id, child_id, vaccine_id, status, doctor_schedule_id |
...
居民选择疫苗 → 选择接种点 → 查看医生排班 → 选择时段预约
                                                    ↓
接种完成 ← 留观结束确认 ← 医生核销接种 ← 签到确认 ← 预约成功（锁定库存）
```
...
```
已预约(1) → 已签到(6) → 预检通过(7) → 留观中(10) → 已完成(2)
                ↓           ↓
            预检未通过(9)  异常终止
```
